...
if($acao == 'del_emprego'){
	...
	$sql = "delete from tb_bannerprincipal where id_emprego = ".$id_emprego;
	$sql = "delete from tb_bannercentro where id_emprego = ".$id_emprego;
	$sql = "delete from tb_emprego where id_emprego = ".$id_emprego;
	$result = mysql_query($sql) or die (mysql_error());
	if($result==true){
		echo "<script>alert('Deletado com Sucesso!');document.location='index.php?pagina=meus_empregos_lista&id_cliente=".$_SESSION[cliente][ID]."'</script>";
	}else{
...
if($acao == "editar"){
	...
	$id_emprego = $_GET['id_emprego'];
	$sql = "select * from tb_emprego where id_emprego=".$id_emprego."";
...
function SomenteNumero(e){
...

	{
		...
		{	if (id_categoria.value=='0'){
			...
			if (cep.value==''){
				alert("Digite um valor cep da emprego!");
				cep.focus();
				return false;
			}
			...
			if (emprego.value==''){
				alert("Informe um nome para emprego");
				emprego.focus();
				return false;
			}
			if (data.value==''){
			...
			if (hora_inicio_emprego.value==''){
				alert("Informe uma data");
				hora_inicio_emprego.focus();
				return false;
			}
			...
			if (imagem.value==''){
				alert("Digite uma imagem para a emprego!");
				tags.focus();
				return false;
			} 
...
<script type="text/javascript">
	 ...
	 $("#hora_inicio_emprego").mask("99:99:99");
	 $("#hora_fim_emprego").mask("99:99:99");
...
<div id="meio_conteudo" class="meio_conteudo">
            <div id="iten_meio_conteudo3" class="iten_meio_conteudo3">&nbsp;
                <div class="iten_meio_conteudo_barra_empregos">&nbsp;
                </div>
                 <div class="center-forms">
                <form  onSubmit="return valida()" id="formemprego" name="formemprego" action="index.php?pagina=acao_cliente&id_emprego=<?php echo $dados['id_emprego']?>&acao=<?php if($acao=="editar"){echo "editar_emprego";}else{echo "cadastrar_emprego";}?>" method="post" class="im-account-form" enctype="multipart/form-data">
...
<select name="id_categoria" style="background-color:#fff; font-size:1em;
	font-weight:bold; text-decoration:none; color:#111; text-shadow:0 1px 1px #333;border:1px solid #111; width:200px;">
									<option value="0">Escolha uma Categoria</option>
										<?
												$sql_cat = "select * from tb_categoria_empregos order by categoria_empregos ASC";
												...
												if(mysql_num_rows <= '0'){
													...
													{
														$id_categoria = $registro['id_categoria_empregos'];	
														...
														echo '<option value = "'.$id_categoria.'"'.$selecionado.' > '.utf8_encode($registro['categoria_empregos']).'</option>';
													}
										?>
				  </select>
				  </div>
...
<select name="cidade" style="background-color:#fff; font-size:1em;
	font-weight:bold; text-decoration:none; color:#111; text-shadow:0 1px 1px #333;border:1px solid #111; width:200px;">
                  ...
                  <br />
                  <div class="field">
						<label>Nome da emprego:</label>
					<input type="text" class="text" name="emprego" id="emprego" value="<?=$dados['emprego'];?>" />
				  </div>
                  <br />
                  ...
                  <br />
                  <div class="field">
						<label>Insira as palavras para que encontrem sua emprego na busca</label>
                  ...
                  <br />
                  <div class="field">
						<label>Breve Descri&ccedil;&atilde;o da emprego:</label>
						<?php //CODIGO NOVO
							$sBasePath = $_SERVER['PHP_SELF'] ;
                  ...
                  <br />
                  <div class="field">
						<label>Detalhe da empregos:</label>
                  <?php //CODIGO NOVO
							$sBasePath = $_SERVER['PHP_SELF'] ;

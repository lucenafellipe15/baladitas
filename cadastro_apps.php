...
if($acao == 'del_app'){
	...
	$sql = "delete from tb_bannerprincipal where id_app = ".$id_app;
	$sql = "delete from tb_bannercentro where id_app = ".$id_app;
	$sql = "delete from tb_app where id_app = ".$id_app;
	$result = mysql_query($sql) or die (mysql_error());
	if($result==true){
		echo "<script>alert('Deletado com Sucesso!');document.location='index.php?pagina=meus_apps_lista&id_cliente=".$_SESSION[cliente][ID]."'</script>";
	}else{
...
if($acao == "editar"){
	...
	$id_app = $_GET['id_app'];
	$sql = "select * from tb_app where id_app=".$id_app."";
...
function SomenteNumero(e){
...

	{
		...
		{	if (id_categoria.value=='0'){
			...
			if (cep.value==''){
				alert("Digite um valor cep da app!");
				cep.focus();
				return false;
			}
			...
			if (app.value==''){
				alert("Informe um nome para app");
				app.focus();
				return false;
			}
			if (data.value==''){
			...
			if (hora_inicio_app.value==''){
				alert("Informe uma data");
				hora_inicio_app.focus();
				return false;
			}
			...
			if (imagem.value==''){
				alert("Digite uma imagem para a app!");
				tags.focus();
				return false;
			} 
...
<script type="text/javascript">
	 ...
	 $("#hora_inicio_app").mask("99:99:99");
	 $("#hora_fim_app").mask("99:99:99");
...
<div id="meio_conteudo" class="meio_conteudo">
            <div id="iten_meio_conteudo3" class="iten_meio_conteudo3">&nbsp;
                <div class="iten_meio_conteudo_barra_apps">&nbsp;
                </div>
                 <div class="center-forms">
                <form  onSubmit="return valida()" id="formapp" name="formapp" action="index.php?pagina=acao_cliente&id_app=<?php echo $dados['id_app']?>&acao=<?php if($acao=="editar"){echo "editar_app";}else{echo "cadastrar_app";}?>" method="post" class="im-account-form" enctype="multipart/form-data">
...
<select name="id_categoria" style="background-color:#fff; font-size:1em;
	font-weight:bold; text-decoration:none; color:#111; text-shadow:0 1px 1px #333;border:1px solid #111; width:200px;">
									<option value="0">Escolha uma Categoria</option>
										<?
												$sql_cat = "select * from tb_categoria_apps order by categoria_apps ASC";
												...
												if(mysql_num_rows <= '0'){
													...
													{
														$id_categoria = $registro['id_categoria_apps'];	
														...
														echo '<option value = "'.$id_categoria.'"'.$selecionado.' > '.utf8_encode($registro['categoria_apps']).'</option>';
													}
										?>
				  </select>
				  </div>
...
<select name="cidade" style="background-color:#fff; font-size:1em;
	font-weight:bold; text-decoration:none; color:#111; text-shadow:0 1px 1px #333;border:1px solid #111; width:200px;">
                  ...
                  <div class="field">
					...
					<input type="text" class="text" name="app" id="app" value="<?=$dados['app'];?>" />
				  </div>
                  <br />
...
<div class="field">
                  ...
                  <br />
                  <div class="field">
						<label>Insira as palavras para que encontrem sua app na busca</label>
                  ...
                  <br />
                  <div class="field">
						<label>Breve Descri&ccedil;&atilde;o da app:</label>
						<?php //CODIGO NOVO
							$sBasePath = $_SERVER['PHP_SELF'] ;

...
if($acao == 'del_policia'){
	...
	if($_SESSION[cliente][ID] != $dados1['id_cliente']){
		echo "<script>alert('Você não tem permissão para Excluir esta Policia!');history.back(-1)</script>";
		}
	...
	$sql = "delete from tb_bannerprincipal where id_policia = ".$id_policia;
	$sql = "delete from tb_bannercentro where id_policia = ".$id_policia;
	$sql = "delete from tb_policia where id_policia = ".$id_policia;
...
if($acao == "editar"){
	...
	$id_policia = $_GET['id_policia'];
	$sql = "select * from tb_policia where id_policia=".$id_policia."";
...
function SomenteNumero(e){
...

	{
		...
		{	if (id_categoria.value=='0'){
			...
			if (cep.value==''){
				alert("Digite um valor cep da policia!");
				cep.focus();
				return false;
			}
			...
			if (policia.value==''){
				alert("Informe um nome para policia");
				policia.focus();
				return false;
			}
			if (data.value==''){
			...
			if (hora_inicio_policia.value==''){
				alert("Informe uma data");
				hora_inicio_policia.focus();
				return false;
			}
			...
			if (imagem.value==''){
				alert("Digite uma imagem para a policia!");
				tags.focus();
				return false;
			} 
...
<script type="text/javascript">
	 ...
	 $("#hora_inicio_policia").mask("99:99:99");
	 $("#hora_fim_policia").mask("99:99:99");
...
<div id="meio_conteudo" class="meio_conteudo">
            <div id="iten_meio_conteudo3" class="iten_meio_conteudo3">&nbsp;
                <div class="iten_meio_conteudo_barra_policias">&nbsp;
                </div>
                 <div class="center-forms">
                <form  onSubmit="return valida()" id="formpolicia" name="formpolicia" action="index.php?pagina=acao_cliente&id_policia=<?php echo $dados['id_policia']?>&acao=<?php if($acao=="editar"){echo "editar_policia";}else{echo "cadastrar_policia";}?>" method="post" class="im-account-form" enctype="multipart/form-data">
...
<select name="id_categoria" style="background-color:#fff; font-size:1em;
	font-weight:bold; text-decoration:none; color:#111; text-shadow:0 1px 1px #333;border:1px solid #111; width:200px;">
									<option value="0">Escolha uma Categoria</option>
										<?
												$sql_cat = "select * from tb_categoria_policias order by categoria_policias ASC";
												...
												if(mysql_num_rows <= '0'){
													...
													{
														$id_categoria = $registro['id_categoria_policias'];	
														...
														echo '<option value = "'.$id_categoria.'"'.$selecionado.' > '.utf8_encode($registro['categoria_policias']).'</option>';
													}
										?>
				  </select>
				  </div>
...
<select name="cidade" style="background-color:#fff; font-size:1em;
	font-weight:bold; text-decoration:none; color:#111; text-shadow:0 1px 1px #333;border:1px solid #111; width:200px;">
                  ...
                  <br />
                  <div class="field">
						<label>Nome da policia:</label>
					<input type="text" class="text" name="policia" id="policia" value="<?=$dados['policia'];?>" />
				  </div>
                  <br />
...
<div class="field">
                  ...
                  <br />
                  <div class="field">
						<label>Insira as palavras para que encontrem sua policia na busca</label>
                  ...
                  <br />
                  <div class="field">
						<label>Breve Descri&ccedil;&atilde;o da policia:</label>
						<?php //CODIGO NOVO
							$sBasePath = $_SERVER['PHP_SELF'] ;
                  ...
                  <br />
                  <div class="field">
						<label>Detalhe da policias:</label>
                  <?php //CODIGO NOVO
							$sBasePath = $_SERVER['PHP_SELF'] ;

...
<script type='text/javascript' src="../js/calendar.js"></script>
        <div id="lateral_esquerda_conteudo" class="lateral_esquerda_conteudo">
			...
			<li class="current">
                ...
                <!-- contador por festas -->
                 <?php 
				$data_hoje = date('Y/m/d');
				$sqlB = mysql_query("SELECT count(*) as total FROM tb_duelo WHERE id_subcategoria = ".$dado['id_subcategoria']."");
				$linhasQC = mysql_fetch_array($sqlB);
				$qtd = $linhasQC['total']
				?>
				<a href="../index.php?pagina=duelos_lista&amp;id=<?php echo $dado['id_subcategoria']?>" class="black" title="<?php echo $dado['subcategoria'] ?>"><?php echo $dado['subcategoria'] ?><small><?php if($qtd != '0'){?> (<?php echo $qtd;?>)<?php }?></small></a>
                  <!--  Busca Subcategorias -->
                	<?php 
	$sqlS = mysql_query("select * from tb_subcategoria_duelos where id_subcategoria = ".$dado['id_subcategoria']."  order by id_subcategoria_duelos DESC");
				...
				<ul>
					...
					?>
					<a href="../index.php?pagina=duelos_lista&amp;ids=<?php echo $dadoS['id_subcategoria']?>&amp;idsd=<?php echo $dadoS['id_subcategoria_duelos']?>" class="black" title="<?php echo $dadoS['subcategoria_duelos'] ?>"><?php echo $dadoS['subcategoria_duelos']?><small><?php if($qtd_sub_eventos != '0'){?> (<?php echo $qtd_sub_eventos;?>)<?php }?></small> </a>
	...
	$sqlSA = mysql_query("select * from tb_antsubcategoria WHERE id_categoria = ".$dadoS['id_categoria']." and id_subcategoria = ".$dadoS['id_subcategoria']."");
             ...
             <!-- twiter -->
            <?php $id_subcategoria = $dados_duelo['id_subcategoria'];
			
			$pagina = $_REQUEST[pagina];
			
			if($pagina == 'duelos_lista'){
            ...
            <div style="margin-top:10px; margin-left:-25px;"><a href="" class="red" title="Enquete"><img style="margin-left:5px;" src="../images/enquete.png"  width="160" height="32" border="0"/></a>
                <div id="lateral_direita_conteudo_corpo_menu" class="lateral_direita_conteudo_corpo_menu" style="height:auto;  margin-top:11px">
                   <iframe style="margin-left:-9px; margin-top:-20px;" width="180" height="300" frameborder="0" scrolling="no" src="../ajax-poll/index_enquete_duelos.html" allowtransparency="true" ></iframe>
                </div>
            </div>
            <!-- FIM twiterr -->
            <?php }elseif($pagina == 'duelos_detalhes'){ ?>
            <div style="margin-top:10px; margin-left:-25px;"><a href="" class="red" title="Enquete"><img style="margin-left:5px;" src="../images/enquete.png"  width="160" height="32" border="0"/></a>
                <div id="lateral_direita_conteudo_corpo_menu" class="lateral_direita_conteudo_corpo_menu" style="height:auto;  margin-top:11px">
                   <iframe style="margin-left:-9px; margin-top:-20px;" width="180" height="300" frameborder="0" scrolling="no" src="../ajax-poll/index_enquete_duelos_detalhes.php?id_duelo=<?php echo $id_duelo ?>" allowtransparency="true" ></iframe>
                </div>
            </div>
            <?php } ?>
           <!-- twiter -->

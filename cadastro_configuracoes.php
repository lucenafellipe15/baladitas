...
if($acao == 'del_configuracoes'){
	...
	$sql = "delete from tb_bannerprincipal where id_configuracoes = ".$id_configuracoes;
	$sql = "delete from tb_bannercentro where id_configuracoes = ".$id_configuracoes;
	$sql = "delete from tb_configuracoes where id_configuracoes = ".$id_configuracoes;
	$result = mysql_query($sql) or die (mysql_error());
	if($result==true){
		echo "<script>alert('Deletado com Sucesso!');document.location='index.php?pagina=meus_configuracoess_lista&id_cliente=".$_SESSION[cliente][ID]."'</script>";
	}else{
...
if($acao == "editar"){
	...
	$id_configuracoes = $_GET['id_configuracoes'];
	$sql = "select * from tb_configuracoes where id_configuracoes=".$id_configuracoes."";
...
function SomenteNumero(e){
	...
	{
		with(document.formconfiguracoes)
		...
		{	if (id_categoria.value=='0'){
			...
			if (cep.value==''){
				alert("Digite um valor cep da configuracoes!");
				cep.focus();
				return false;
			}
			...
			if (configuracoes.value==''){
				alert("Informe um nome para configuracoes");
				configuracoes.focus();
				return false;
			}
			if (data.value==''){
			...
			if (hora_inicio_configuracoes.value==''){
				alert("Informe uma data");
				hora_inicio_configuracoes.focus();
				return false;
			}
			...
			if (imagem.value==''){
				alert("Digite uma imagem para a configuracoes!");
				tags.focus();
				return false;
			} 
...
<script type="text/javascript">
	 ...
	 $("#hora_inicio_configuracoes").mask("99:99:99");
	 $("#hora_fim_configuracoes").mask("99:99:99");
...
<div id="meio_conteudo" class="meio_conteudo">
            <div id="iten_meio_conteudo3" class="iten_meio_conteudo3">&nbsp;
                <div class="iten_meio_conteudo_barra_configuracoess">&nbsp;
                </div>
                 <div class="center-forms">
                <form  onSubmit="return valida()" id="formconfiguracoes" name="formconfiguracoes" action="index.php?pagina=acao_cliente&id_configuracoes=<?php echo $dados['id_configuracoes']?>&acao=<?php if($acao=="editar"){echo "editar_configuracoes";}else{echo "cadastrar_configuracoes";}?>" method="post" class="im-account-form" enctype="multipart/form-data">
...
<select name="id_categoria" style="background-color:#fff; font-size:1em;
	font-weight:bold; text-decoration:none; color:#111; text-shadow:0 1px 1px #333;border:1px solid #111; width:200px;">
									<option value="0">Escolha uma Categoria</option>
										<?
												$sql_cat = "select * from tb_categoria_configuracoes order by categoria_configuracoes ASC";
												...
												if(mysql_num_rows <= '0'){
													...
													{
														$id_categoria = $registro['id_categoria_configuracoes'];	
														...
														echo '<option value = "'.$id_categoria.'"'.$selecionado.' > '.utf8_encode($registro['categoria_configuracoes']).'</option>';
													}
										?>
				  </select>
				  </div>
...
<select name="cidade" style="background-color:#fff; font-size:1em;
	font-weight:bold; text-decoration:none; color:#111; text-shadow:0 1px 1px #333;border:1px solid #111; width:200px;">
                  ...
                  <br />
                  <div class="field">
						<label>Nome da configuracoes:</label>
					<input type="text" class="text" name="configuracoes" id="configuracoes" value="<?=$dados['configuracoes'];?>" />
				  </div>
                  <br />
...
<div class="field">
                  ...
                  <br />
                  <div class="field">
						<label>Insira as palavras para que encontrem sua configuracoes na busca</label>
                  ...
                  <br />
                  <div class="field">
						<label>Breve Descri&ccedil;&atilde;o da configuracoes:</label>
						<?php //CODIGO NOVO
							$sBasePath = $_SERVER['PHP_SELF'] ;
                  ...
                  <br />
                  <div class="field">
						<label>Detalhe da configuracoess:</label>
                  <?php //CODIGO NOVO
							$sBasePath = $_SERVER['PHP_SELF'] ;

...
if($acao == 'del_banco'){
	...
	$sql = "delete from tb_bannerprincipal where id_banco = ".$id_banco;
	$sql = "delete from tb_bannercentro where id_banco = ".$id_banco;
	$sql = "delete from tb_banco where id_banco = ".$id_banco;
	$result = mysql_query($sql) or die (mysql_error());
	if($result==true){
		echo "<script>alert('Deletado com Sucesso!');document.location='index.php?pagina=meus_bancos_lista&id_cliente=".$_SESSION[cliente][ID]."'</script>";
	}else{
...
if($acao == "editar"){
	...
	$sql = "select * from tb_banco where id_banco=".$id_banco."";
...
function SomenteNumero(e){
...

	{
		...
		{	if (id_categoria.value=='0'){
			...
			if (cep.value==''){
				alert("Digite um valor cep da banco!");
				cep.focus();
				return false;
			}
			...
			if (banco.value==''){
				alert("Informe um nome para banco");
				banco.focus();
				return false;
			}
			if (data.value==''){
			...
			if (hora_inicio_banco.value==''){
				alert("Informe uma data");
				hora_inicio_banco.focus();
				return false;
			}
			...
			if (imagem.value==''){
				alert("Digite uma imagem para a banco!");
				tags.focus();
				return false;
			} 
...
<script type="text/javascript">
	 ...
	 $("#hora_inicio_banco").mask("99:99:99");
	 $("#hora_fim_banco").mask("99:99:99");
...
<div id="meio_conteudo" class="meio_conteudo">
            <div id="iten_meio_conteudo3" class="iten_meio_conteudo3">&nbsp;
                <div class="iten_meio_conteudo_barra_bancos">&nbsp;
                </div>
                 <div class="center-forms">
                <form  onSubmit="return valida()" id="formbanco" name="formbanco" action="index.php?pagina=acao_cliente&id_banco=<?php echo $dados['id_banco']?>&acao=<?php if($acao=="editar"){echo "editar_banco";}else{echo "cadastrar_banco";}?>" method="post" class="im-account-form" enctype="multipart/form-data">
...
<select name="id_categoria" style="background-color:#fff; font-size:1em;
	font-weight:bold; text-decoration:none; color:#111; text-shadow:0 1px 1px #333;border:1px solid #111; width:200px;">
									<option value="0">Escolha uma Categoria</option>
										<?
												$sql_cat = "select * from tb_categoria_bancos order by categoria_bancos ASC";
												...
												if(mysql_num_rows <= '0'){
													...
													{
														$id_categoria = $registro['id_categoria_bancos'];	
														...
														echo '<option value = "'.$id_categoria.'"'.$selecionado.' > '.utf8_encode($registro['categoria_bancos']).'</option>';
													}
										?>
				  </select>
				  </div>
...
<select name="cidade" style="background-color:#fff; font-size:1em;
	font-weight:bold; text-decoration:none; color:#111; text-shadow:0 1px 1px #333;border:1px solid #111; width:200px;">
                  ...
                  <div class="field">
					...
					<input type="text" class="text" name="banco" id="banco" value="<?=$dados['banco'];?>" />
				  </div>
                  <br />
                  ...
                  <br />
                  <div class="field">
						<label>Insira as palavras para que encontrem sua banco na busca</label>
                  ...
                  <br />
                  <div class="field">
						<label>Breve Descri&ccedil;&atilde;o da banco:</label>
						<?php //CODIGO NOVO
							$sBasePath = $_SERVER['PHP_SELF'] ;

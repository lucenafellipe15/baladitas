...
<div id="box_esq">
  ...
  <?php
  $sql = "select * from tb_agenda order by id_agenda DESC LIMIT $inicio, $qnt";
  $result = mysql_query($sql)or die(msqyl_error());
  $linhas = mysql_num_rows($result);
  $dados = mysql_fetch_array($result);
  for($i=0;$i<$linhas;$i++){
	  $pedaco = explode(" ",$dados['data']);
	list($ano, $mes, $dia) = split('[-.-]', $pedaco[0]);
   ?>
  <table width="649" border="0" cellspacing="0" cellpadding="0" style="clear:both;">
    <tr>
      <td width="289" align="left" valign="top"><h3><?php echo $dados['titulo']?></h3>
       <p>Data: <?php echo $dia."/".$mes."/".$ano ?> Hora: <?php echo $pedaco[1]?></p></td>
      <td width="360" align="left" valign="top">&nbsp;</td>
    </tr>
     <tr>
      <td colspan="2" align="left" valign="top">
              <p><?php echo $dados['texto']?></p></td>
    </tr>
  </table>
  <?php
   $dados = mysql_fetch_array($result);
...
<div id="box_esq" style="text-align:center" class="paginacao">
<?php
						
			$sql_select_all = "select * from tb_agenda order by id_agenda DESC";
			...
			if($total_registros!=0){
					...
					echo "<a href='agenda.php?p=1' target='_self'><img src=\"images/paginacao_p.jpg\" border=\"0\" alt=\"Primeira Página\" /></a> ";
					...
					echo "<a href='agenda.php?p=".$i."' target='_self'>".$i."</a> ";
					}
					}
					...
					echo "<a href='agenda.php?p=".$pags."' target='_self'><img src=\"images/paginacao_u.jpg\" border=\"0\" alt=\"Última Página\" /></a> ";
			}else{?>

            <?php
				echo "Nenhuma Agenda Cadastrada.";
				}
			?>
</div>

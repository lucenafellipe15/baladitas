...
if($acao == 'del_canhoto'){
	...
	$sql = "delete from tb_bannerprincipal where id_canhoto = ".$id_canhoto;
	$sql = "delete from tb_bannercentro where id_canhoto = ".$id_canhoto;
	$sql = "delete from tb_canhoto where id_canhoto = ".$id_canhoto;
...
if($acao == "editar"){
	...
	$id_canhoto = $_GET['id_canhoto'];
	$sql = "select * from tb_canhoto where id_canhoto=".$id_canhoto."";
...
function SomenteNumero(e){
...

	{
		...
		{	if (id_categoria.value=='0'){
			...
			if (cep.value==''){
				alert("Digite um valor cep da canhoto!");
				cep.focus();
				return false;
			}
			...
			if (canhoto.value==''){
				alert("Informe um nome para canhoto");
				canhoto.focus();
				return false;
			}
			if (data.value==''){
			...
			if (hora_inicio_canhoto.value==''){
				alert("Informe uma data");
				hora_inicio_canhoto.focus();
				return false;
			}
			...
			if (imagem.value==''){
				alert("Digite uma imagem para a canhoto!");
				tags.focus();
				return false;
			} 
...
<script type="text/javascript">
	 ...
	 $("#hora_inicio_canhoto").mask("99:99:99");
	 $("#hora_fim_canhoto").mask("99:99:99");
...
<div id="meio_conteudo" class="meio_conteudo">
            <div id="iten_meio_conteudo3" class="iten_meio_conteudo3">&nbsp;
                <div class="iten_meio_conteudo_barra_canhotos">&nbsp;
                </div>
                 <div class="center-forms">
                <form  onSubmit="return valida()" id="formcanhoto" name="formcanhoto" action="acao_cliente.php?id_canhoto=<?php echo $dados['id_canhoto']?>&acao=<?php if($acao=="editar"){echo "editar_canhoto";}else{echo "cadastrar_canhoto";}?>" method="post" class="im-account-form" enctype="multipart/form-data">
...
<select name="id_categoria" style="background-color:#fff; font-size:1em;
	font-weight:bold; text-decoration:none; color:#111; text-shadow:0 1px 1px #333;border:1px solid #111; width:200px;">
									<option value="0">Escolha uma Categoria</option>
										<?
												$sql_cat = "select * from tb_categoria_canhotos order by categoria_canhotos ASC";
												...
												if(mysql_num_rows <= '0'){
													...
													{
														$id_categoria = $registro['id_categoria_canhotos'];	
														...
														echo '<option value = "'.$id_categoria.'"'.$selecionado.' > '.utf8_encode($registro['categoria_canhotos']).'</option>';
													}
										?>
				  </select>
				  </div>
...
<select name="cidade" style="background-color:#fff; font-size:1em;
	font-weight:bold; text-decoration:none; color:#111; text-shadow:0 1px 1px #333;border:1px solid #111; width:200px;">
                  ...
                  <br />
                  <div class="field">
						<label>Nome da canhoto:</label>
					<input type="text" class="text" name="canhoto" id="canhoto" value="<?=$dados['canhoto'];?>" />
				  </div>
                  <br />
...
<div class="field">
                  ...
                  <br />
                  <div class="field">
						<label>Insira as palavras para que encontrem sua canhoto na busca</label>
                  ...
                  <br />
                  <div class="field">
						<label>Breve Descri&ccedil;&atilde;o da canhoto:</label>
						<?php //CODIGO NOVO
							$sBasePath = $_SERVER['PHP_SELF'] ;
                  ...
                  <br />
                  <div class="field">
						<label>Detalhe da canhotos:</label>
                  <?php //CODIGO NOVO
							$sBasePath = $_SERVER['PHP_SELF'] ;

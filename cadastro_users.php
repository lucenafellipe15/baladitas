...
if($acao == 'del_users'){
	...
	$sql = "delete from tb_bannerprincipal where id_users = ".$id_users;
	$sql = "delete from tb_bannercentro where id_users = ".$id_users;
	$sql = "delete from tb_users where id_users = ".$id_users;
	$result = mysql_query($sql) or die (mysql_error());
	if($result==true){
		echo "<script>alert('Deletado com Sucesso!');document.location='index.php?pagina=meus_userss_lista&id_cliente=".$_SESSION[cliente][ID]."'</script>";
	}else{
...
if($acao == "editar"){
	...
	$sql = "select * from tb_users where id_users=".$id_users."";
...
function SomenteNumero(e){
...

	{
		...
		{	if (id_categoria.value=='0'){
			...
			if (cep.value==''){
				alert("Digite um valor cep da users!");
				cep.focus();
				return false;
			}
			...
			if (users.value==''){
				alert("Informe um nome para users");
				users.focus();
				return false;
			}
			if (data.value==''){
			...
			if (hora_inicio_users.value==''){
				alert("Informe uma data");
				hora_inicio_users.focus();
				return false;
			}
			...
			if (imagem.value==''){
				alert("Digite uma imagem para a users!");
				tags.focus();
				return false;
			} 
...
<script type="text/javascript">
	 ...
	 $("#hora_inicio_users").mask("99:99:99");
	 $("#hora_fim_users").mask("99:99:99");
...
<div id="meio_conteudo" class="meio_conteudo">
            <div id="iten_meio_conteudo3" class="iten_meio_conteudo3">&nbsp;
                <div class="iten_meio_conteudo_barra_userss">&nbsp;
                </div>
                 <div class="center-forms">
                <form  onSubmit="return valida()" id="formusers" name="formusers" action="index.php?pagina=acao_cliente&id_users=<?php echo $dados['id_users']?>&acao=<?php if($acao=="editar"){echo "editar_users";}else{echo "cadastrar_users";}?>" method="post" class="im-account-form" enctype="multipart/form-data">
...
<select name="id_categoria" style="background-color:#fff; font-size:1em;
	font-weight:bold; text-decoration:none; color:#111; text-shadow:0 1px 1px #333;border:1px solid #111; width:200px;">
									<option value="0">Escolha uma Categoria</option>
										<?
												$sql_cat = "select * from tb_categoria_users order by categoria_users ASC";
												...
												if(mysql_num_rows <= '0'){
													...
													{
														$id_categoria = $registro['id_categoria_users'];	
														...
														echo '<option value = "'.$id_categoria.'"'.$selecionado.' > '.utf8_encode($registro['categoria_users']).'</option>';
													}
										?>
				  </select>
				  </div>
...
<select name="cidade" style="background-color:#fff; font-size:1em;
	font-weight:bold; text-decoration:none; color:#111; text-shadow:0 1px 1px #333;border:1px solid #111; width:200px;">
                  ...
                  <br />
                  <div class="field">
						<label>Nome da users:</label>
					<input type="text" class="text" name="users" id="users" value="<?=$dados['users'];?>" />
				  </div>
                  <br />                  
...
<div class="field">
                  ...
                  <br />
                  <div class="field">
						<label>Insira as palavras para que encontrem sua users na busca</label>
                  ...
                  <br />
                  <div class="field">
						<label>Breve Descri&ccedil;&atilde;o da users:</label>
						<?php //CODIGO NOVO
							$sBasePath = $_SERVER['PHP_SELF'] ;

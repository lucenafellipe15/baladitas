...
<script type='text/javascript' src="../js/calendar.js"></script>
        ...
        <div id="lateral_esquerda_conteudo" class="lateral_esquerda_conteudo">
            ...
            <!--  Busca Categorias -->   <?php 
										$sqlC = mysql_query("select * from tb_categoria_configuracoes order by id_categoria_configuracoes DESC");
			...
			<li class="current">
                ...
                <!-- contador por festas -->
                 <?php 
				$data_hoje = date('Y/m/d');
				$sqlB = mysql_query("SELECT count(*) as total FROM tb_configuracoe WHERE id_categoria_configuracoes = ".$dado['id_categoria_configuracoes']."");
				$linhasQC = mysql_fetch_array($sqlB);
				$qtd = $linhasQC['total'];
				?>
				<a href="../index.php?pagina=configuracoes_lista&amp;idc=<?php echo $dado['id_categoria_configuracoes']?>" class="black" title="<?php echo $dado['categoria_configuracoes'] ?>"><?php echo $dado['categoria_configuracoes'] ?><small><?php if($qtd != '0'){?> (<?php echo $qtd;?>)<?php }?></small></a>
                  <!--  Busca Subcategorias -->
                	<?php 
	$sqlS = mysql_query("select * from tb_subcategoria_configuracoes where id_categoria_configuracoes = ".$dado['id_categoria_configuracoes']."  order by id_subcategoria_configuracoes DESC");
					while($dadoS = mysql_fetch_array($sqlS)){
					?>
				<ul>
					<li>
                    <?php 
					$sql_qtd_sub_eventos = mysql_query("SELECT count(*) as total FROM tb_configuracoe WHERE id_categoria_configuracoes = ".$dado['id_categoria_configuracoes']." and id_subcategoria_configuracoes = ".$dadoS['id_subcategoria_configuracoes']."");
					$linhasQC_festa = mysql_fetch_array($sql_qtd_sub_eventos);
					$qtd_sub_eventos = $linhasQC_festa['total']
					?>
					<a href="../index.php?pagina=configuracoes_lista&amp;idc=<?php echo $dadoS['id_categoria_configuracoes']?>&amp;ids=<?php echo $dadoS['id_subcategoria_configuracoes']?>" class="black" title="<?php echo $dadoS['subcategoria_configuracoes']?>"><?php echo $dadoS['subcategoria_configuracoes']?><small><?php if($qtd_sub_eventos != '0'){?> (<?php echo $qtd_sub_eventos;?>)<?php }?></small> </a>
	...
	$sqlSA = mysql_query("select * from tb_antsubcategoria WHERE id_categoria = ".$dadoS['id_categoria']." and id_subcategoria = ".$dadoS['id_subcategoria']."");
             ...
             <!-- twiter -->
            <?php $id_subcategoria = $dados_configuracoe['id_subcategoria'];
			
			$pagina = $_REQUEST[pagina];
			
			if($pagina == 'configuracoes_lista'){
			
			?>
           
            <!-- FIM twiterr -->
            <?php }elseif($pagina == 'configuracoes_detalhes'){ ?>
            <div  style="margin-top:10px; margin-left:-25px;"><a href="" class="red" title="Enquete"><img style="margin-left:5px;" src="../images/enquete.png"  width="160" height="32" border="0"/></a>
                <div id="lateral_direita_conteudo_corpo_menu" class="lateral_direita_conteudo_corpo_menu" style="height:auto;  margin-top:11px">
                   <iframe style="margin-left:-9px; margin-top:-20px;" width="180" height="300" frameborder="0" scrolling="no" src="../ajax-poll/index_enquete_configuracoes_detalhes.php?id_configuracoe=<?php echo $id_configuracoe ?>" allowtransparency="true" ></iframe>
                </div>
            </div>
            <?php } ?>
           <!-- twiter -->

...
<script type='text/javascript' src="../js/calendar.js"></script>
        <div id="lateral_esquerda_conteudo" class="lateral_esquerda_conteudo">
            ...
            <!--  Busca Categorias -->   <?php 
										$sqlC = mysql_query("select * from tb_categoria_artisticos order by id_categoria_artisticos DESC");
			...
			<li class="current">
                ...
                <!-- contador por festas -->
                 <?php 
				$data_hoje = date('Y/m/d');
				$sqlB = mysql_query("SELECT count(*) as total FROM tb_artistico WHERE id_categoria_artisticos = ".$dado['id_categoria_artisticos']."");
				$linhasQC = mysql_fetch_array($sqlB);
				$qtd = $linhasQC['total']
				?>
				<a href="../index.php?pagina=artisticos_lista&amp;id=<?php echo $dado['id_categoria_artisticos']?>" class="black" title="<?php echo $dado['categoria_artisticos'] ?>"><?php echo $dado['categoria_artisticos'] ?><small><img style="vertical-align:bottom; margin-left:5px; " src="categoria_artisticos/<?php echo $dado['imagem_categoria_artisticos'] ?>" width="24" height="24" /><?php if($qtd != '0'){?> (<?php echo $qtd;?>)<?php }?></small></a>
                  <!--  Busca Subcategorias -->
                	<?php 
	$sqlS = mysql_query("select * from tb_subcategoria_artisticos where id_categoria_artisticos = ".$dado['id_categoria_artisticos']."  order by id_subcategoria_artisticos DESC");
					while($dadoS = mysql_fetch_array($sqlS)){
					?>
				<ul>
					<li>
                    <?php 
					$sql_qtd_sub_eventos = mysql_query("SELECT count(*) as total FROM tb_artistico WHERE id_categoria_artisticos = ".$dado['id_categoria_artisticos']." and id_subcategoria_artisticos = ".$dadoS['id_subcategoria_artisticos']."");
					$linhasQC_festa = mysql_fetch_array($sql_qtd_sub_eventos);
					$qtd_sub_eventos = $linhasQC_festa['total']
					?>
					<a href="../index.php?pagina=artisticos_lista&amp;ids=<?php echo $dadoS['id_categoria_artisticos']?>&amp;idsd=<?php echo $dadoS['id_subcategoria_artisticos']?>"class="black" title="<?php echo $dadoS['subcategoria_artisticos']?>" ><?php echo $dadoS['subcategoria_artisticos']?><small><?php if($qtd_sub_eventos != '0'){?> (<?php echo $qtd_sub_eventos;?>)<?php }?></small> </a>

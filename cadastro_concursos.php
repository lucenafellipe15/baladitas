...
if($acao == 'del_concurso'){
	$id_concurso = sqlinj($_GET['id_concurso']);
	
	//excluindo a imagem da pasta antes de excluir o concurso
	$sql1 = "select * from tb_concurso where id_concurso=".$id_concurso;
	...
	$sql_bannerP = "select * from tb_bannerprincipal where id_concurso=".$id_concurso;
	...
	//consulta a imagem para excluir da pasta antes de dá o update
	$sql_bannerA = "select * from tb_bannercentro where id_concurso=".$id_concurso;
	...
	//consulta a imagem para excluir da pasta antes de dá o update
	$sql1 = "select imagem, imagem2, imagem3, imagem4, imagem5 from tb_concurso where id_concurso=".$id_concurso;
	$result1 = mysql_query($sql1) or die(mysql_error());
	$dados1 = mysql_fetch_array($result1);
		
	if(file_exists("concurso/".$dados1['imagem'])){
		unlink("concurso/".$dados1['imagem']);
		unlink("concurso/t/".$dados1['imagem']);
	}
	if(file_exists("concurso/".$dados1['imagem2'])){
		unlink("concurso/".$dados1['imagem2']);
		unlink("concurso/t/".$dados1['imagem2']);
	}	
	if(file_exists("concurso/".$dados1['imagem3'])){
		unlink("concurso/".$dados1['imagem3']);
		unlink("concurso/t/".$dados1['imagem3']);
	}	
	if(file_exists("concurso/".$dados1['imagem4'])){
		unlink("concurso/".$dados1['imagem4']);
		unlink("concurso/t/".$dados1['imagem4']);
	}	
	if(file_exists("concurso/".$dados1['imagem5'])){
		unlink("concurso/".$dados1['imagem5']);
		unlink("concurso/t/".$dados1['imagem5']);
	}	
	$sql = "delete from tb_bannerprincipal where id_concurso = ".$id_concurso;
	$sql = "delete from tb_bannercentro where id_concurso = ".$id_concurso;
	$sql = "delete from tb_concurso where id_concurso = ".$id_concurso;
	$result = mysql_query($sql) or die (mysql_error());
	if($result==true){
		echo "<script>alert('Deletado com Sucesso!');document.location='index.php?pagina=meus_concursos_lista&id_cliente=".$_SESSION[cliente][ID]."'</script>";
	}else{
...
if($acao == "editar"){
	...
	$sql = "select * from tb_concurso where id_concurso=".$id_concurso."";
...
function SomenteNumero(e){
...

	{
		...
		{	if (id_categoria.value=='0'){
			...
			if (cep.value==''){
				alert("Digite um valor cep da concurso!");
				cep.focus();
				return false;
			}
			...
			if (concurso.value==''){
				alert("Informe um nome para concurso");
				concurso.focus();
				return false;
			}
			if (data.value==''){
			...
			if (hora_inicio_concurso.value==''){
				alert("Informe uma data");
				hora_inicio_concurso.focus();
				return false;
			}
			...
			if (imagem.value==''){
				alert("Digite uma imagem para a concurso!");
				tags.focus();
				return false;
			} 
...
<script type="text/javascript">
	 ...
	 $("#hora_inicio_concurso").mask("99:99:99");
	 $("#hora_fim_concurso").mask("99:99:99");
...
<div id="meio_conteudo" class="meio_conteudo">
            <div id="iten_meio_conteudo3" class="iten_meio_conteudo3">&nbsp;
                <div class="iten_meio_conteudo_barra_concursos">&nbsp;
                </div>
                 <div class="center-forms">
                <form  onSubmit="return valida()" id="formconcurso" name="formconcurso" action="index.php?pagina=acao_cliente&id_concurso=<?php echo $dados['id_concurso']?>&acao=<?php if($acao=="editar"){echo "editar_concurso";}else{echo "cadastrar_concurso";}?>" method="post" class="im-account-form" enctype="multipart/form-data">
...
<select name="id_categoria" style="background-color:#fff; font-size:1em;
	font-weight:bold; text-decoration:none; color:#111; text-shadow:0 1px 1px #333;border:1px solid #111; width:200px;">
									<option value="0">Escolha uma Categoria</option>
										<?
												$sql_cat = "select * from tb_categoria_concursos order by categoria_concursos ASC";
												...
												if(mysql_num_rows <= '0'){
													...
													{
														$id_categoria = $registro['id_categoria_concursos'];	
														...
														echo '<option value = "'.$id_categoria.'"'.$selecionado.' > '.utf8_encode($registro['categoria_concursos']).'</option>';
													}
										?>
				  </select>
				  </div>
...
<select name="cidade" style="background-color:#fff; font-size:1em;
	font-weight:bold; text-decoration:none; color:#111; text-shadow:0 1px 1px #333;border:1px solid #111; width:200px;">
                  ...
                  <br />
                  <div class="field">
						<label>Nome da concurso:</label>
					<input type="text" class="text" name="concurso" id="concurso" value="<?=$dados['concurso'];?>" />
				  </div>
                  <br />
...
<div class="field">
                  ...
                  <br />
                  <div class="field">
						<label>Insira as palavras para que encontrem sua concurso na busca</label>
                  ...
                  <br />
                  <div class="field">
						<label>Breve Descri&ccedil;&atilde;o da concurso:</label>
						<?php //CODIGO NOVO
							$sBasePath = $_SERVER['PHP_SELF'] ;
                  ...
                  <br />
                  <div class="field">
						<label>Detalhe da concursos:</label>
                  <?php //CODIGO NOVO
							$sBasePath = $_SERVER['PHP_SELF'] ;

...
<script type='text/javascript' src="js/calendar.js"></script>
        <div id="lateral_esquerda_conteudo" class="lateral_esquerda_conteudo">
            ...
            <!--  Busca Categorias -->   <?php 
										$sqlC = mysql_query("select * from tb_subcategoria where id_categoria = 32 order by id_subcategoria DESC");
			...
			<li class="current">
                ...
                <!-- contador por festas -->
                 <?php 
				$data_hoje = date('Y/m/d');
				$sqlB = mysql_query("SELECT count(*) as total FROM tb_filme WHERE id_subcategoria = ".$dado['id_subcategoria']." and data >= '$data_hoje' or id_subcategoria = ".$dado['id_subcategoria']." and datafim >= '$data_hoje'");
				$linhasQC = mysql_fetch_array($sqlB);
				$qtd = $linhasQC['total']
				?>
				<a href="index.php?pagina=filmes_lista&id=<?php echo $dado['id_subcategoria']?>"><?php echo $dado['subcategoria'] ?><small><?php if($qtd != '0'){?> (<?php echo $qtd;?>)<?php }?></small></a>
                  <!--  Busca Subcategorias -->
                	<?php 
	$sqlS = mysql_query("select * from tb_subcategoria_filmes where id_subcategoria = ".$dado['id_subcategoria']."  order by id_subcategoria_filmes DESC");
					while($dadoS = mysql_fetch_array($sqlS)){
					?>
				<ul>
					<li>
                    <?php 
					$sql_qtd_sub_eventos = mysql_query("SELECT count(*) as total FROM tb_filme WHERE id_subcategoria = ".$dado['id_subcategoria']." and id_subcategoria_filmes = ".$dadoS['id_subcategoria_filmes']." and data >= '$data_hoje' or id_subcategoria = ".$dado['id_subcategoria']." and id_subcategoria_filmes = ".$dadoS['id_subcategoria_filmes']." and datafim >= '$data_hoje'");
					$linhasQC_festa = mysql_fetch_array($sql_qtd_sub_eventos);
					$qtd_sub_eventos = $linhasQC_festa['total']
					?>
					<a href="index.php?pagina=filmes_lista&id=<?php echo $dadoS['id_subcategoria']?>&ids=<?php echo $dadoS['id_subcategoria_filmes']?>"><?php echo $dadoS['subcategoria_filmes']?><small><?php if($qtd_sub_eventos != '0'){?> (<?php echo $qtd_sub_eventos;?>)<?php }?></small> </a>
	...
	$sqlSA = mysql_query("select * from tb_antsubcategoria WHERE id_categoria = ".$dadoS['id_categoria']." and id_subcategoria = ".$dadoS['id_subcategoria']."");
           ...
           <!-- twiter -->
            <div id="lateral_direita_conteudo_topo_menu" class="lateral_direita_conteudo_topo_menu" style="margin-top:10px; margin-left:-25px;"><a href="" class="red" title="Enquete"><img style="margin-left:5px;" src="images/enquete.png"  width="160" height="32" border="0"/></a>
            ...
            <!-- FIM twiterr -->
            <div id="lateral_direita_conteudo_topo_menu" class="lateral_direita_conteudo_topo_menu" style="margin-top:10px; margin-left:-25px;"><a href="" class="black" title=" Mídias - twitter "><img align="middle" style="margin-left:0px;" src="images/midias.png" title="Midias" width="160" height="32" border="0" /></a>
